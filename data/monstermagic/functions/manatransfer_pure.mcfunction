...
execute as @e[tag=manaPurifier] at @s if entity @e[type=armor_stand,scores={Mana=1..},distance=..3,sort=nearest,limit=1] run execute as @e[type=armor_stand,scores={Mana=0}] at @s if block ~ ~-1 ~ sea_lantern store success score @e[type=vex,tag=manaPurifier,sort=nearest,limit=1] DidAddMana run scoreboard players add @s Mana 1
execute as @e[tag=manaPurifier,scores={DidAddMana=1}] at @s if entity @e[type=armor_stand,scores={Mana=1..},distance=..3,sort=nearest,limit=1] if entity @e[type=armor_stand,scores={Mana=1..},distance=..3,sort=nearest,limit=1] run scoreboard players remove @e[type=armor_stand,scores={Mana=1..},distance=..3,sort=nearest,limit=1] Mana 1
